...
class Id :public Exp {
  ...
  public:
    ...
    Value eval(Frame*ST) override { return Value(ST->lookup(val)); }
...
class BoolOp :public Exp {
  ...
  public:
    ...
    Value eval(Frame*ST) override{
      if (op == AND) return Value(left->eval(ST).tf() && right->eval(ST).tf());
      else return Value(left->eval(ST).tf() || right->eval(ST).tf());
    };
};
